...
export default function EditEntryScreen() {
  ...
  return (
    ...
    >
      ...
      {/* No Share button here any more — N531/#962 moved sharing to the
          entry row's own 3-dot menu on the day view (`EntryMenuSheet`), which
          is one tap CLOSER than this screen was (row → menu → Share, against
          row → this screen → Share). The gate it carried (`entrySyncState` →
          `shareBlockedReason`) moved with it, unchanged: sharing still sends
          what the SERVER holds, so an unsynced or edited-since row is still
