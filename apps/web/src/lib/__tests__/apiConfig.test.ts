 ...
 * `NEXT_PUBLIC_API_URL` falls back to `localhost` or throws.
 *
 * Tests `resolveApiBaseUrl` directly as a pure function of
 * `(rawEnvValue, isDevelopment)`, rather than mocking `process.env.NODE_ENV`
 * in `apiConfig.ts` itself — this is what makes both branches testable
 * without flipping a real process-wide value mid-suite.
 */
 ...
 * vitest process. Measured directly: `vitest run` sets `NODE_ENV=test`, not
 * `development` — see `apiConfig.ts`'s own doc comment for why the gate is
 * `!== "production"` rather than `=== "development"`. Importing the module
 * here is itself the "don't break local dev/tests" regression check: a
