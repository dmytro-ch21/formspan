...
it('a fresh install ends up at the current schema version', async () => {
  ...
  expect(row.user_version).toBe(45);
});
...
it('re-running migrate on the SAME database is idempotent', async () => {
  ...
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});
...
it('a fresh install has the biometric_hr_synced ledger (N477)', async () => {
  // v34. The local half of "don't offer this session's heart-rate window to
  // the server twice" — see CREATE_BIOMETRIC_HR_SYNCED's own doc comment in
  // db.ts for why this is keyed on session_id rather than a HealthKit uuid.
  const db = await migratedFixture();
  const cols = (
    db.raw.prepare('PRAGMA table_info(biometric_hr_synced)').all() as { name: string }[]
  ).map((c) => c.name);
  expect(cols).toEqual(expect.arrayContaining(['user_id', 'session_id', 'synced_at']));
});

it('a device already stamped 33 gains the biometric_hr_synced table (N477)', async () => {
  // Every dev machine and every installed build is stamped 33 as of this
  // ticket, and `migrate()` returns early at `current >= SCHEMA_VERSION` —
  // reverting SCHEMA_VERSION to 33 is what this catches. Without the table,
  // lib/biometricSync.ts's ledger reads/writes would throw the moment the
  // existing HealthKit toggle is ever turned on.
  const db = await migratedFixture();
  db.raw.exec('DROP TABLE biometric_hr_synced; PRAGMA user_version = 33;');
  ...
  expect(tables).toContain('biometric_hr_synced');
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

it('re-running the N477 biometric_hr_synced migration is not an error', async () => {
  const db = await migratedFixture();
  db.raw.exec('PRAGMA user_version = 33;');
  ...
  expect(tables.filter((name) => name === 'biometric_hr_synced')).toHaveLength(1);
});

it('a fresh install has the health_connect_enrichment ledger (N478)', async () => {
  // v35. The retry ledger for Android biometric enrichment — see
  // CREATE_HEALTH_CONNECT_ENRICHMENT's own doc comment in db.ts for why this
  // is a retry ledger (overwritten per attempt) rather than a dedup table
  // like healthkit_imports/biometric_hr_synced above.
  const db = await migratedFixture();
  const cols = (
    db.raw.prepare('PRAGMA table_info(health_connect_enrichment)').all() as { name: string }[]
  ).map((c) => c.name);
  expect(cols).toEqual(
    expect.arrayContaining(['user_id', 'session_id', 'hr_source', 'sample_count', 'attempted_at']),
  );
});

it('a device already stamped 34 gains the health_connect_enrichment table (N478)', async () => {
  // Every dev machine and every installed build predating this ticket is
  // stamped 34 as of N477 (immediately above) merging first, and
  // `migrate()` returns early at `current >= SCHEMA_VERSION` — reverting
  // SCHEMA_VERSION to 34 is what this catches. Without the table,
  // syncHealthConnectBiometrics's ledger reads/writes would throw the
  // moment the toggle was ever turned on.
  const db = await migratedFixture();
  db.raw.exec('DROP TABLE health_connect_enrichment; PRAGMA user_version = 34;');
  ...
  expect(tables).toContain('health_connect_enrichment');
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

it('re-running the N478 health_connect_enrichment migration is not an error', async () => {
  const db = await migratedFixture();
  db.raw.exec('PRAGMA user_version = 34;');
  ...
  expect(tables.filter((name) => name === 'health_connect_enrichment')).toHaveLength(1);
});

it('a fresh install has the detected_activities ledger (N479)', async () => {
  // v36. See CREATE_DETECTED_ACTIVITIES's own doc comment in db.ts for why
  // `dismissed_at` is the only mutable column — everything else is a cache
  // of what the platform reported, re-cross-referenced against
  // `local_sessions` at read time rather than trusted from here.
  const db = await migratedFixture();
  const cols = (
    db.raw.prepare('PRAGMA table_info(detected_activities)').all() as { name: string }[]
  ).map((c) => c.name);
  expect(cols).toEqual(
    expect.arrayContaining([
      'user_id',
...
it('a device already stamped 35 gains the detected_activities table (N479)', async () => {
  // Every dev machine and every installed build predating this ticket is
  // stamped 35 as of N478 (immediately above) merging first, and
  // `migrate()` returns early at `current >= SCHEMA_VERSION` — reverting
  // SCHEMA_VERSION to 35 is what this catches. Without the table, Today's
  // detected-activity read/write would throw the moment the athlete's
  // HealthKit/Health Connect toggle is on.
  const db = await migratedFixture();
  db.raw.exec('DROP TABLE detected_activities; PRAGMA user_version = 35;');
  ...
  expect(tables).toContain('detected_activities');
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

it('re-running the N479 detected_activities migration is not an error', async () => {
  const db = await migratedFixture();
  db.raw.exec('PRAGMA user_version = 35;');
  ...
  expect(tables.filter((name) => name === 'detected_activities')).toHaveLength(1);
});

it('a fresh install has hr_source/attempted_at on biometric_hr_synced (N511/#893)', async () => {
  const db = await migratedFixture();
  const cols = (
    db.raw.prepare('PRAGMA table_info(biometric_hr_synced)').all() as { name: string }[]
  ).map((c) => c.name);
  expect(cols).toEqual(
    expect.arrayContaining(['user_id', 'session_id', 'synced_at', 'hr_source', 'attempted_at']),
  );
});

it('a device already stamped 36 gains hr_source/attempted_at, and existing rows backfill honestly (N511/#893)', async () => {
  // Every dev machine and every installed build predating this ticket is
  // stamped 36, and `migrate()` returns early at `current >= SCHEMA_VERSION`
  // — reverting SCHEMA_VERSION to 36 is what this catches. Without these
  // columns, `lib/biometricSync.ts`'s ledger read/write would throw the
  // moment the next foreground pass ran.
  const db = await migratedFixture();
  db.raw.exec(
    `INSERT INTO biometric_hr_synced (user_id, session_id, synced_at) VALUES ('u1', 's1', '2026-08-01T00:00:00.000Z');
     ALTER TABLE biometric_hr_synced DROP COLUMN hr_source;
     ALTER TABLE biometric_hr_synced DROP COLUMN attempted_at;
  ...
  // The pre-existing row backfills to 'none' (see CREATE_BIOMETRIC_HR_SYNCED's
  // and the v37 migration block's own doc comments on why 'none', not
  // 'window', is the honest choice here), and attempted_at copies the
  // pre-existing synced_at rather than staying empty.
  const row = db.raw
    .prepare('SELECT hr_source, attempted_at FROM biometric_hr_synced WHERE session_id = ?')
    .get('s1') as { hr_source: string; attempted_at: string };
  expect(row.hr_source).toBe('none');
...
it('a device already stamped 37 gains time_of_day_minutes (N126/#520)', async () => {
  ...
  // — reverting SCHEMA_VERSION to 37 is what this catches. Without this
  // column, every sync-pull writing a server-set time would throw with "no
  // column named time_of_day_minutes", and every local plan write would be
  // the same "day only" it already was.
  const db = await migratedFixture();
  db.raw.exec(
    `INSERT INTO planned_sessions
       (id, user_id, day, sport, workout_id, notes, created_at, updated_at, dirty, remote)
     VALUES ('p1', 'u1', '2026-08-05', 'strength', NULL, '', '2026-08-01T00:00:00.000Z', '2026-08-01T00:00:00.000Z', 0, 1);
     ALTER TABLE planned_sessions DROP COLUMN time_of_day_minutes;
  ...
  expect(cols).toContain('time_of_day_minutes');
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });

  // No backfill: a plan made before this column existed genuinely has no
  // time to give it. NULL is the honest answer, not a guessed 0 (midnight).
  const row = db.raw
    .prepare('SELECT time_of_day_minutes FROM planned_sessions WHERE id = ?')
    .get('p1') as { time_of_day_minutes: number | null };
  expect(row.time_of_day_minutes).toBeNull();
});

it('a fresh install’s planned_sessions has time_of_day_minutes from the start', async () => {
  const db = await migratedFixture();
  const cols = (
    db.raw.prepare('PRAGMA table_info(planned_sessions)').all() as { name: string }[]
  ).map((c) => c.name);
...
it('re-running the N511 biometric_hr_synced retry-ledger migration is not an error', async () => {
  const db = await migratedFixture();
  ...
  expect(cols.filter((c) => c === 'hr_source')).toHaveLength(1);
  expect(cols.filter((c) => c === 'attempted_at')).toHaveLength(1);
});

it('a device already stamped 38 gains collapsed_json, defaulting to nothing collapsed (N530/#961)', async () => {
  // Same shape as the 37 test above: every installed build predating this
  // ticket is stamped 38, and `migrate()` returns early at
  // `current >= SCHEMA_VERSION` — so reverting SCHEMA_VERSION to 38 is what
  // this catches. Without the column, `readCollapsedGroups` on the session
  // screen's `load` throws "no such column" on every open.
  const db = await migratedFixture();
  db.raw.exec(
    `INSERT INTO local_sessions
       (id, user_id, workout_id, sport, name, intent, started_at, ended_at, notes,
        sets_json, dirty, remote, updated_at)
     VALUES ('s1', 'u1', NULL, 'strength', 'Legs', 'normal', '2026-08-01T00:00:00.000Z', NULL, '',
             '[]', 0, 1, '2026-08-01T00:00:00.000Z');
     ALTER TABLE local_sessions DROP COLUMN collapsed_json;
  ...
  expect(cols).toContain('collapsed_json');
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });

  // The only honest backfill: nobody has tapped Done on a session that
  // predates the button.
  const row = db.raw.prepare('SELECT collapsed_json FROM local_sessions WHERE id = ?').get('s1') as {
    collapsed_json: string;
  };
  expect(row.collapsed_json).toBe('[]');
});


it('a device already stamped 39 gains foods.shared_by and shared_at (N532/#963)', async () => {
  // Every installed build predating this ticket is stamped 39 (N530 took 39
  // first; this is 40), and `migrate()` returns early at
  // `current >= SCHEMA_VERSION` — so reverting SCHEMA_VERSION to 39 is what
  // this catches. Without these columns the very next foods pull throws on
  // its first row ("no column named shared_by"), which is exactly the
  // failure `cacheFoods`'s own history records once going unnoticed in
  // production.
  const db = await migratedFixture();
  db.raw.exec(
    `INSERT INTO foods (id, user_id, kind, name, serving_label, kcal, created_at, cached_at)
     VALUES ('f1', 'u1', 'food', 'Oats', '50 g', 180, '2026-08-01T00:00:00.000Z', '2026-08-01T00:00:00.000Z');
     ALTER TABLE foods DROP COLUMN shared_by;
     ALTER TABLE foods DROP COLUMN shared_at;
  ...
  // No backfill: nothing on the device knows which cached rows were shared.
  // NULL is "not shared, as far as this device knows" until the next pull.
  const row = db.raw
    .prepare('SELECT shared_by, shared_at FROM foods WHERE id = ?')
    .get('f1') as { shared_by: string | null; shared_at: string | null };
  expect(row.shared_by).toBeNull();
  expect(row.shared_at).toBeNull();
});

it('re-running the N532 share-provenance migration is not an error', async () => {
  const db = await migratedFixture();
  db.raw.exec('PRAGMA user_version = 39;');
  ...
  expect(cols.filter((c) => c === 'shared_by')).toHaveLength(1);
  expect(cols.filter((c) => c === 'shared_at')).toHaveLength(1);
});

it('a device already stamped 41 gains position on food_entries, backfilled to the order it already showed (N553/#1019)', async () => {
  // Same class as N124/N113's `category` above: a real ALTER with no
  // `CREATE TABLE IF NOT EXISTS` backstop, so a stamped-41 device without
  // this branch keeps a `food_entries` with no `position` and `localEntries`
  // — which now orders by it — throws "no such column" on every open of the
  // food day.
  //
  // The BACKFILL is the half that matters to the athlete, and it is why this
  // test arranges rows rather than just checking the column exists. Position
  // is seeded from `logged_at, id`, which is exactly what `localEntries`
  // sorted by BEFORE this ticket — so the first load after the update shows
  // the same order as the last load before it. A migration that got this
  // wrong would silently reshuffle every meal on every device, which would
  // read as the new reorder feature being broken rather than as an upgrade
  // bug.
  //
  // The ids are deliberately in the OPPOSITE order to `logged_at`, so a
  // backfill that sorted by id alone (or that forgot to sort at all and used
  // rowid) lands visibly wrong rather than accidentally right.
  const db = await migratedFixture();
  db.raw.exec(`
    ALTER TABLE food_entries DROP COLUMN position;
    INSERT INTO food_entries
      (id, user_id, eaten_on, meal, name, servings, serving_label, kcal,
       protein_g, carb_g, fat_g, notes, logged_at, updated_at, dirty, remote)
    VALUES
      ('e3', 'u1', '2026-08-05', 'breakfast', 'Eggs',   1, '100 g', 180, 25, 0, 8, '', '2026-08-05T07:00:00Z', '2026-08-05T07:00:00Z', 0, 1),
      ('e2', 'u1', '2026-08-05', 'breakfast', 'Toast',  1, '100 g', 180, 25, 0, 8, '', '2026-08-05T07:05:00Z', '2026-08-05T07:05:00Z', 0, 1),
      ('e1', 'u1', '2026-08-05', 'breakfast', 'Coffee', 1, '100 g', 180, 25, 0, 8, '', '2026-08-05T07:10:00Z', '2026-08-05T07:10:00Z', 0, 1),
      ('d2', 'u1', '2026-08-05', 'dinner',    'Steak',  1, '100 g', 180, 25, 0, 8, '', '2026-08-05T19:00:00Z', '2026-08-05T19:00:00Z', 0, 1),
      ('d1', 'u1', '2026-08-05', 'dinner',    'Rice',   1, '100 g', 180, 25, 0, 8, '', '2026-08-05T19:02:00Z', '2026-08-05T19:02:00Z', 0, 1),
      ('o1', 'u2', '2026-08-05', 'breakfast', 'Oats',   1, '100 g', 180, 25, 0, 8, '', '2026-08-05T07:00:00Z', '2026-08-05T07:00:00Z', 0, 1);
  ...
  const rows = db.raw
    .prepare('SELECT id, meal, position FROM food_entries ORDER BY id')
    .all() as { id: string; meal: string; position: number }[];
  const at = (id: string) => rows.find((r) => r.id === id)?.position;

  // The order the device was already showing, on the 1024 grid.
  expect(at('e3')).toBe(1024);
  expect(at('e2')).toBe(2048);
  expect(at('e1')).toBe(3072);

  // Per MEAL, not per day: dinner starts over at 1024 rather than continuing
  // breakfast's numbering. Two entries in different meals sharing a value is
  // ordinary — nothing ever compares them.
  expect(at('d2')).toBe(1024);
  expect(at('d1')).toBe(2048);

  // Per USER too. Another athlete signed into this device has their own list;
  // counting their rows would push this one's numbering along.
  expect(at('o1')).toBe(1024);
});

it('re-running the N553 position migration is not an error, and does not renumber a row the athlete moved', async () => {
  // The idempotence half is the same check every migration here gets. The
  // second half is specific and is why the backfill has no `WHERE position =
  // 0` guard: 0 is a LEGITIMATE position — dragging the second row of a meal
  // above the first is 1024 - 1024 — so a guard keyed on 0 would re-number
  // exactly the row the athlete had just moved. Re-running the unguarded
  // version instead recomputes from `logged_at, id`, which is the same answer
  // it gave the first time.
  const db = await migratedFixture();
  db.raw.exec('PRAGMA user_version = 41;');
  ...
  expect(cols.filter((name) => name === 'position')).toHaveLength(1);
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

it('a fresh install has position on the food log, so the CREATE and the ALTER agree', async () => {
  // The trap this closes: `CREATE TABLE IF NOT EXISTS` is a no-op against an
  // existing table, so a column added ONLY in the v42 branch works on every
  // upgraded device and is missing on every fresh install — and the suite
  // above would stay green, because it upgrades. `addColumnIfMissing` makes
  // the branch a no-op on a fresh install rather than the other way round,
  // which is what makes both paths reach the same shape.
  const db = await migratedFixture();
  const cols = (
    db.raw.prepare('PRAGMA table_info(food_entries)').all() as {
      name: string;
      notnull: number;
      dflt_value: string | null;
    }[]
  );
  const position = cols.find((c) => c.name === 'position');
  expect(position).toBeDefined();
  expect(position?.notnull).toBe(1);
  expect(position?.dflt_value).toBe('0');
});

it('a fresh install has the body read cache, holding neither photo links, notes nor an outbox flag (N568/#1129)', async () => {
  // A read cache, not an outbox: no `dirty`, no `remote`. And narrow on
  // purpose — `photo_url` is a presigned link that expires, and `notes` is
  // free text neither panel fact needs.
  const db = await migratedFixture();
  const colsOf = (t: string) =>
    (db.raw.prepare(`PRAGMA table_info(${t})`).all() as { name: string }[]).map((c) => c.name).sort();
  expect(colsOf('body_checkins_cache')).toEqual(['fetched_at', 'measured_on', 'user_id', 'weight_kg']);
  expect(colsOf('body_phases_cache')).toEqual(
    ['ended_on', 'fetched_at', 'id', 'kind', 'started_on', 'target_on', 'target_weight_kg', 'user_id'],
  );
  expect(colsOf('body_cache_fetches')).toEqual(['fetched_at', 'kind', 'user_id', 'window_from', 'window_to']);
});

it('a device already stamped 42 gains the body read cache tables (N568/#1129)', async () => {
  // Every installed build predating this ticket is stamped 42, and `migrate()`
  // returns early at `current >= SCHEMA_VERSION` — so leaving SCHEMA_VERSION at
  // 42 is what this catches. Without the tables, Today's check-in refresh would
  // swallow a failed cache write forever and the day panel's Body block would
  // say "not available" in every dead spot, for the life of the install.
  const db = await migratedFixture();
  db.raw.exec(`
    DROP TABLE body_checkins_cache;
    DROP TABLE body_phases_cache;
    DROP TABLE body_cache_fetches;
  ...
  expect(tables).toEqual(expect.arrayContaining(['body_checkins_cache', 'body_phases_cache', 'body_cache_fetches']));
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

it('re-running the N568 body cache migration is not an error, and keeps what was cached', async () => {
  const db = await migratedFixture();
  db.raw.exec(`
    INSERT INTO body_checkins_cache (user_id, measured_on, weight_kg, fetched_at)
      VALUES ('u1', '2026-09-10', 82.1, '2026-09-11T08:00:00.000Z');
    INSERT INTO body_cache_fetches (user_id, kind, fetched_at, window_from, window_to)
      VALUES ('u1', 'checkins', '2026-09-11T08:00:00.000Z', '2026-08-12', '2026-09-11');
    PRAGMA user_version = 42;
  `);
  ...
  for (const t of ['body_checkins_cache', 'body_phases_cache', 'body_cache_fetches']) {
    expect(tables.filter((name) => name === t)).toHaveLength(1);
  }
  expect(db.raw.prepare('SELECT count(*) AS n FROM body_checkins_cache').get()).toEqual({ n: 1 });
  expect(db.raw.prepare('SELECT count(*) AS n FROM body_cache_fetches').get()).toEqual({ n: 1 });
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

it('a fresh install has the steps store: a reading per day, and the latest answer, with no outbox flag (N569/#1130)', async () => {
  // Local only — no `dirty`, no `remote`. And two tables on purpose: a refusal
  // lives in `steps_read_state` and never as a `daily_steps` row, so nothing that
  // is not a count can be read back as one.
  const db = await migratedFixture();
  const colsOf = (t: string) =>
    (db.raw.prepare(`PRAGMA table_info(${t})`).all() as { name: string }[]).map((c) => c.name).sort();
  expect(colsOf('daily_steps')).toEqual(['day', 'read_at', 'source', 'steps', 'user_id']);
  expect(colsOf('steps_read_state')).toEqual(['checked_at', 'source', 'state', 'user_id']);
});

it('a device already stamped 43 gains the steps tables (N569/#1130)', async () => {
  // Every installed build predating this ticket is stamped 43, and `migrate()`
  // returns early at `current >= SCHEMA_VERSION` — so leaving SCHEMA_VERSION at
  // 43 is what this catches. Without the tables every steps read would fail its
  // write, silently, and VOLA would say "not available" for the life of the install.
  const db = await migratedFixture();
  db.raw.exec(`
    DROP TABLE daily_steps;
    DROP TABLE steps_read_state;
  ...
  expect(tables).toEqual(expect.arrayContaining(['daily_steps', 'steps_read_state']));
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

it('re-running the N569 steps migration is not an error, and keeps what was read', async () => {
  const db = await migratedFixture();
  db.raw.exec(`
    INSERT INTO daily_steps (user_id, day, steps, source, read_at)
      VALUES ('u1', '2026-09-12', 8412, 'healthkit', '2026-09-12T21:05:00.000Z');
    INSERT INTO steps_read_state (user_id, state, source, checked_at)
      VALUES ('u1', 'read', 'healthkit', '2026-09-12T21:05:00.000Z');
    PRAGMA user_version = 43;
  `);
  ...
  for (const t of ['daily_steps', 'steps_read_state']) {
    expect(tables.filter((name) => name === t)).toHaveLength(1);
  }
  expect(db.raw.prepare('SELECT steps FROM daily_steps').get()).toEqual({ steps: 8412 });
  expect(db.raw.prepare('SELECT state FROM steps_read_state').get()).toEqual({ state: 'read' });
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

/** Every outbox a stuck row can live in — `STUCK_ROW_SOURCES`' tables (N565/#1108). */
const STUCK_TABLES = ['local_sessions', 'workout_cache', 'planned_sessions', 'food_entries', 'sequences'];

const triggerNames = (db: Awaited<ReturnType<typeof migratedFixture>>) =>
  (db.raw.prepare("SELECT name FROM sqlite_master WHERE type='trigger'").all() as { name: string }[])
    .map((t) => t.name)
    .sort();

it('a fresh install records the code and when a row got stuck, on every stuck-row outbox (N565/#1108)', async () => {
  const db = await migratedFixture();
  for (const t of STUCK_TABLES) {
    const cols = (db.raw.prepare(`PRAGMA table_info(${t})`).all() as { name: string }[]).map((c) => c.name);
    expect(cols).toEqual(expect.arrayContaining(['last_error', 'last_error_code', 'stuck_since']));
  }
  expect(triggerNames(db)).toEqual(
    STUCK_TABLES.flatMap((t) => [`${t}_error_code_cleared`, `${t}_stuck_since`]).sort(),
  );
});

it('a device already stamped 44 gains the columns and triggers, and rows ALREADY stuck get no invented age or code (N565/#1108)', async () => {
  // Every installed build before this ticket is stamped 44. Its stuck rows were
  // refused at a moment it never wrote down, with a code it never kept — so the
  // honest upgrade leaves both NULL, and the report calls them unknown.
  const db = await migratedFixture();
  for (const t of STUCK_TABLES) {
    db.raw.exec(`
      DROP TRIGGER ${t}_stuck_since;
      DROP TRIGGER ${t}_error_code_cleared;
      ALTER TABLE ${t} DROP COLUMN stuck_since;
      ALTER TABLE ${t} DROP COLUMN last_error_code;
    `);
  }
  db.raw.exec(`
    INSERT INTO local_sessions
      (id, user_id, workout_id, sport, name, started_at, ended_at, notes, sets_json, dirty, remote, deleted_at, updated_at, last_error)
      VALUES ('s-old', 'u1', NULL, 'strength', 'Leg Day', '2026-01-01T10:00:00Z', NULL, '', '[]', 1, 1, NULL, '2026-01-01T10:00:00Z', 'refused in January'),
             ('s-new', 'u1', NULL, 'strength', 'Push', '2026-09-01T10:00:00Z', NULL, '', '[]', 1, 1, NULL, '2026-09-01T10:00:00Z', NULL);
    INSERT INTO food_entries
      (id, user_id, eaten_on, meal, name, servings, serving_label, kcal, notes, position, logged_at, updated_at, dirty, remote, deleted_at, last_error)
      VALUES ('f-old', 'u1', '2026-01-02', 'lunch', 'Rice', 1, 'serving', 100, '', 0, '2026-01-02T12:00:00Z', '2026-01-02T12:00:00Z', 0, 0, NULL, 'refused');
    PRAGMA user_version = 44;
  `);

  await migrate(db as never);

  expect(db.raw.prepare(`SELECT stuck_since, last_error_code FROM local_sessions WHERE id = 's-old'`).get())
    .toEqual({ stuck_since: null, last_error_code: null });
  expect(db.raw.prepare(`SELECT stuck_since, last_error_code FROM food_entries WHERE id = 'f-old'`).get())
    .toEqual({ stuck_since: null, last_error_code: null });

  // And the upgraded device is not left without the triggers: its NEXT refusal is timed.
  db.raw.exec(`UPDATE local_sessions SET last_error = 'refused now', last_error_code = 'invalid_input' WHERE id = 's-new'`);
  expect(
    (db.raw.prepare(`SELECT stuck_since FROM local_sessions WHERE id = 's-new'`).get() as { stuck_since: string | null })
      .stuck_since,
  ).toEqual(expect.any(String));
  expect(triggerNames(db)).toHaveLength(STUCK_TABLES.length * 2);
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});

it('re-running the N565 migration is not an error, and does not restart a clock already running', async () => {
  const db = await migratedFixture();
  db.raw.exec(`
    INSERT INTO local_sessions
      (id, user_id, workout_id, sport, name, started_at, ended_at, notes, sets_json, dirty, remote, deleted_at, updated_at,
       last_error, last_error_code, stuck_since)
      VALUES ('s1', 'u1', NULL, 'strength', 'Leg Day', '2026-09-01T10:00:00Z', NULL, '', '[]', 1, 1, NULL, '2026-09-01T10:00:00Z',
              'refused', 'invalid_input', '2026-09-01T11:00:00.000Z');
    PRAGMA user_version = 44;
  `);

  await expect(migrate(db as never)).resolves.toBeUndefined();

  expect(db.raw.prepare(`SELECT stuck_since, last_error_code FROM local_sessions WHERE id = 's1'`).get())
    .toEqual({ stuck_since: '2026-09-01T11:00:00.000Z', last_error_code: 'invalid_input' });
  expect(triggerNames(db)).toHaveLength(STUCK_TABLES.length * 2);
  expect(db.raw.prepare('PRAGMA user_version').get()).toEqual({ user_version: 45 });
});
