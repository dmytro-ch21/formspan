...
//	24 runs, wall clock    387s          453s   (+17%)
...
// Conn is the subset of pgx's connection types the FIXTURE HOOKS need — enough
// that a package's existing seed/remove functions keep their bodies unchanged.
// It is satisfied by *pgx.Conn, *pgxpool.Conn, *pgxpool.Pool and pgx.Tx.
//
// It is deliberately NOT the type of any lock function. A session-level advisory
// lock belongs to the connection that took it, and a pooled connection is handed
// straight back: an unlock through a pool lands on some other session and does
// nothing, while a successful `TryLock` through a pool parks the lock on an
// arbitrary pooled session nothing will ever release — wedging every other lane
// until the pool closes, and telling the victim to go looking for a test binary
// that does not exist. The lock functions therefore take a concrete `*pgx.Conn`,
// so that is a type error rather than a comment somebody has to read.
type Conn interface {
...
// crashed run cannot wedge the next one.
...
func Unlock(ctx context.Context, conn *pgx.Conn) error {
...
// holderPID is the Postgres backend pid of the connection Lock succeeded on —
// written only by recordHolder, read only by AssertHeld.
//
// It exists because "is the lock held?" is the WRONG question for a guard to
// ask. `pg_try_advisory_lock` returning false proves only that SOMEBODY holds
// the key, and in a fleet that somebody is routinely a neighbouring binary — so
// a guard built on it passes vacuously in exactly the conditions the lock exists
// for. (Found by the parallel review of #426's own guard, which had the same
// hole; the fix is theirs.) The guard has to ask WHO, and this is the answer it
// compares against.
var holderPID int

func recordHolder(ctx context.Context, conn *pgx.Conn) error {
...
// AssertHeld fails unless THIS binary holds its database's fixture lock. Skips
// when TEST_DATABASE_URL is unset, like every other Postgres test.
//
...
//
// **It asks WHO holds the lock, not WHETHER it can be taken**, and the
// difference is the whole guard. `pg_try_advisory_lock` failing proves only that
// somebody holds the key — and in the fleet this exists for, that somebody is
// routinely a neighbouring binary. A guard built on it therefore passes
// vacuously in precisely the conditions the lock exists to handle: delete the
// TestMain, run while a colleague's suite is up, and the probe still cannot take
// the lock, so the guard still goes green. (This hole was found by the parallel
// review of #426's guard, which had it too; the pg_locks fix is theirs.)
//
// So it checks two things against `pg_locks`: that there is exactly ONE holder,
// and that the holder is the backend Lock succeeded on in this process.
func AssertHeld(t *testing.T) {
	...
	if holderPID == 0 {
		t.Fatal("this binary never took the fixture lock: TestMain must call testdb.Main " +
			"(or testdb.MainWithFixtures). Without it, a second test binary sharing this " +
			"database will delete these fixtures out from under an in-flight test — see #454.")
	}
	ctx := context.Background()
	conn, err := pgx.Connect(ctx, url)
	if err != nil {
	...
	// classid is the first key, objid the second AS AN OID (so a negative int32
	// wraps), objsubid 2 for the two-key form. Verified against Postgres 16
	// rather than assumed.
	var holders []int32
	if err := conn.QueryRow(ctx, `
		...
		WHERE locktype = 'advisory'
		  AND classid = $1::oid
		  AND objid = (`+LockScopeSQL+`)::oid
		  AND objsubid = 2
		  AND granted`, LockKey).Scan(&holders); err != nil {
		t.Fatalf("ask pg_locks who holds the fixture lock: %v", err)
	}

	switch {
	case len(holders) == 0:
		t.Fatalf("nothing holds this database's fixture lock, though this process took it "+
			"on backend %d — the holding connection has gone, so a second test binary can "+
			"now delete these fixtures mid-test. See #454.", holderPID)
	case len(holders) > 1:
		t.Fatalf("%d connections hold this database's fixture lock (%v) — it is not "+
			"exclusive, so it is not protecting anything. See #454.", len(holders), holders)
	case int(holders[0]) != holderPID:
		t.Fatalf("this database's fixture lock is held by backend %d, not by this process's "+
			"backend %d. Some OTHER test binary owns these rows right now, and this one is "+
			"writing them anyway. See #454.", holders[0], holderPID)
	}
}
