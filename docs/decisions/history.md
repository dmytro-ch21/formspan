...
- First end-to-end vertical slice, Phase 1 (backend) done: `internal/modules/activity` (idempotent create, self + admin-scoped list) and real backend admin authorization (`ADMIN_USER_IDS`, `RequireAdmin`) exist and are curl-verified. Phases 2–5 (mobile Clerk auth, mobile offline logging + sync, web display, admin real-data wiring) are not started — nothing calls these endpoints yet outside manual verification, and `apps/admin`'s frontend gate still uses `ADMIN_EMAILS`, not the new `ADMIN_USER_IDS` convention.
