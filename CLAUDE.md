...
- `assets/brand/` — the VOLA brand kit, and the **source of truth** for brand identity: logos, app-icon and splash masters, 25 UI icons, and `design-tokens.json`. All SVG — the rasters in `apps/mobile/assets/images/` are *generated* from these, so edit the SVG and regenerate, never the PNG. `pnpm run check:brand-rasters` (in `verify` and CI) holds that: each raster's bytes are pinned, and each must still depict its master within a measured tolerance. `scripts/brand-rasters.json` lists every raster, its master, and the re-export steps. UI icons use `currentColor`, so recolour via CSS/props rather than by forking the file.
