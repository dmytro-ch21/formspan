...
- [ ] **N9** — Grip has no `mixed` or `hook`, so the picker is withheld from hinges, carries and olympic lifts — the movements where grip matters MOST. A deadlifter cannot record how they pull. Needs those two values plus a decision on whether `mixed` needs a side. (#PR)
- [ ] **N10** — Web cannot author grip, only display it. Its `SetRow` takes a name and booleans, not the `Exercise`, so gating a picker needs a prop change. Logging is a phone thing, so this is for correcting a session at a desk. (#PR)
