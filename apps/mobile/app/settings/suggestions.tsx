...
export default function SuggestionSettingsScreen() {
  ...
  /**
   * `null` until read, like `dismissed`. Started at the optimistic default,
   * an athlete who has switched suggestions off saw the master flash ON and
   * snap OFF on every visit — a settings screen briefly stating the opposite
   * of the setting.
   */
  const [master, setMaster] = useState<boolean | null>(null);
  const [off, setOff] = useState<ReadonlySet<string> | null>(null);
  ...
  // One gate for the whole screen: nothing here can be stated before it is
  // known, and three separate spinners would be worse than one wait.
  const ready = master !== null && off !== null && dismissed !== null;

  return (
    <ScrollView contentContainerStyle={styles.scroll} testID="suggestion-settings">
      <Stack.Screen options={{ title: 'Suggestions' }} />

      {!ready && <ActivityIndicator style={styles.loading} />}
      ...
      <View style={[styles.group, master === false && styles.groupOff]}>
        {sports.map((m, i) => (
          <Row
            key={m.key}
            label={labelFor(modules, m.key)}
            // NOT `master && …`. Rendering every row off while the master
            // is off loses "which ones did I turn off" as completely as
            // hiding them would — which is the thing the comment above says
            // this block exists to preserve, and which the note below
            // promises. `disabled` and `groupOff` carry the master state.
            value={!off?.has(m.key)}
            disabled={master === false}
      ...
      {dismissed !== null && dismissed.size === 0 ? (
        <Text style={styles.note}>
        ...
        <View style={styles.group}>
          {[...(dismissed ?? [])].map((id, i) => (
            <View key={id} style={[styles.row, i < (dismissed?.size ?? 0) - 1 && styles.rowDivided]}>
