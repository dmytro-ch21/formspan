...
| 7 | Author a weekly training theme | `/dashboard/calendar` (`setTheme`) | `lib/themes.ts`: *"read-only on the phone… no write path here on purpose"* | **closed by N82 (PENDING_PR_N82)** |
...
| `functional-scenarios.md` | Themes "Authored on web, read on the phone… **no way to author one on the phone**, deliberately" — **corrected on the exclusivity by N82 (PENDING_PR_N82)**: `WeekPlanner` (Plan tab) now authors a theme for the shown week, title only, matching web's own field; web keeps the richer whole-month view |
...
| [#416](https://github.com/dmytro-ch21/formspan/issues/416) | Theme authoring | 7 — **closed by N82 (PENDING_PR_N82)** |
