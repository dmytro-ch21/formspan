...
### Reduced on mobile — allowed, but two are worth watching
...
| Browse session history | Mobile reads local SQLite, and the server pull is `limit: 20` (`sessionStore.ts`). On a fresh install, older sessions are unreachable on the phone at all. |
...
| [#417](https://github.com/dmytro-ch21/formspan/issues/417) | Curriculum authoring | 8 |
