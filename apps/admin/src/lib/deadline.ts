/**
 * The deadline every admin request runs under — N159 (#576).
 *
 * Before this, `adminFetch` had none: a hung backend held a Server Component
 * render until the server runtime gave up, minutes later.
 *
 * **A copy of `apps/web/src/lib/deadline.ts`, and the numbers are the phone's.**
 * `apps/mobile/lib/authedFetch.ts` owns the reasoning for both, and
 * `scripts/check-timeout-parity.py` fails the build if this copy, web's and
 * the phone's drift apart. Copied rather than shared, the way `trace.ts` is:
 * this repo has no shared package. Read web's copy for why a timeout is not an
 * `AbortError` and why the deadline covers the body.
 */
