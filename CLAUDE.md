...
- `assets/brand/` — the VOLA brand kit, and the **source of truth** for brand identity: logos, app-icon and splash masters, 25 UI icons, and `design-tokens.json`. All SVG — the rasters in `apps/mobile/assets/images/` were *exported* from these by hand, and there is no generator script, so edit the SVG and re-export, never the PNG. `pnpm run check:brand-rasters` (in `verify` and CI) holds that: each raster's bytes are pinned, and each must still depict its master within a measured tolerance. `scripts/brand-rasters.json` lists every raster, its master, and the re-export steps. UI icons use `currentColor`, so recolour via CSS/props rather than by forking the file.
...
It must report every check run it expects and exit 0. **How many that is is
deliberately not written here.** The command's first line prints
`check runs on this commit: N (expected M)`, and `M` is however many jobs the
workflows declare today, which the script derives rather than assumes: it
cross-checks the derived set against `EXPECTED_CHECK_RUNS` in
`scripts/check-ci-checks.py` and **fails loudly if the two disagree**, so adding
a CI job means changing that constant in the same commit rather than discovering
later that the bar quietly moved. `M` is therefore the constant, or the tool
refuses. (The numeral used to live in this sentence, and it lagged the code at
**every** change it went through — 5 to 6 at the 2026-08-20 probe-PR #401
correction below, then 6 to 8 by N166 (#543), which this sentence did not follow
until H18 noticed the gap on 2026-09-11. The one part that survived both drifts
was the instruction to read it from `EXPECTED_CHECK_RUNS` itself rather than
trust a numeral in prose — so H24 (#1094) kept the instruction and removed the
numeral. Do not put it back: nothing checks prose against the constant, and the
tool already tells you the number at the moment you need it.)
...
- The related but distinct case: Colima **up but contended**, not down —
  `engine/internal/worker`'s egress-broker tests failing with "broker did not
  become ready" under the full package suite while passing instantly in
  isolation (N470/#799). Not a downed apparatus, and not the test's own logic;
  this host's Docker VM is small and shared by every concurrent session, and a
  slow-starting sidecar container reads exactly like a broken test until you
  check what else was running against the same daemon at the same time. See
  "Known gotchas" above and `docs/decisions/history.md`'s N470 entry.
...
- Railway: the real project is **still named `formspan`** — the VOLA rename covered the repo and code, not the external service accounts (Railway, Clerk). Don't "correct" it in docs until it's actually renamed in the Railway dashboard. Its `staging` environment runs `api`, `web`, `admin` and a real Postgres, all live, and the three app services auto-deploy on every push to `main`. Its `production` environment has no services — no production Postgres, no production API. Both verified 2026-09-14; `docs/architecture/deployment.md` has the table and a read-only re-verification recipe. Note that Nixpacks-built services (`web`, `admin`) need `NIXPACKS_INSTALL_CMD` set to bypass corepack — see `railway/web.toml`. An **unrelated pre-existing project, `dynamic-trust`** (service `medical-portal-api`), sits in the same Railway account — it is not ours; never touch it.
...
- **This host's Colima VM is also small (2 CPUs / 2GiB — `~/.colima/default/colima.yaml`) and shared by every concurrent Docker use on the machine**, which per this repo's own worktree/parallel-agent convention routinely means several sessions' `engine` tests at once (N470/#799, `engine/internal/worker/egress.go`). `internal/worker`'s egress-broker tests each start a sidecar container that does a cold `go run .` compile (no persisted build cache), and under real contention from other concurrent sessions on this shared daemon that measurably took **30s** for a single isolated test with no local parallelism at all — confirmed live by watching `docker ps` show fresh, rotating `RunID`s that weren't this session's own. This is the "apparatus flakiness" class the "Verify that a check can fail" section already names for a downed Colima; a slow-but-up Colima under contention is the same category with a different symptom (a readiness timeout, not instant failure) — see that section and `docs/decisions/history.md`'s N470 entry for the fuller writeup, including a related cancellation-race bug in the same file that could leak the broker's own Docker network and was fixed alongside this.
