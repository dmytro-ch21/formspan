...
- [x] **N4** — Any set can be given a duration, and a set that carries one is timed. The rule used to refuse the EXERCISE (`weight_reps` gets no timer); it now refuses only the GUESS — an explicit duration outranks the load type, nothing is invented for a set nobody timed. Circuits genuinely fall out: `canRun` consults only `workSeconds`, proved by an integration test wiring the real `workSecondsFor` rather than the stub the interval-run suite injects. A duration is a timer TARGET, not a measure — `measuresForSet` is untouched, so a timed squat is still a weight×reps set. (#259)
