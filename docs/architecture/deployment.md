...
**Neither incident above would have flipped a healthcheck red, because `/v1/healthz` (the only one that existed) is liveness-only — no DB, no dependencies.** A failed or silently-skipped predeploy step left a stale environment serving with the process itself perfectly alive, which is exactly what a liveness probe cannot see by design. N163/#540 (2026-09) adds `/v1/readyz` (`backend/cmd/api/readyz.go`): it pings the database on a short bounded deadline and compares `schema_migrations`'s recorded version (and dirty flag) against the highest migration file embedded in the running binary, returning `503` on any mismatch. `railway/api.toml`'s `healthcheckPath` now points there instead of at `/healthz` — Railway's `[deploy]` block has exactly one healthcheck slot, so it gets the stricter of the two checks (a live-but-not-ready process must not receive traffic either). AI/LLM provider health is explicitly excluded from `/readyz` — see the comment on `readinessChecker` in that file, and the N163 history entry.
