...
- [x] **F3** — Read all 142. **Ten were wrong, in both directions**: seven single-implement movements (`svend`, `hip-thrust`, `russian-twist`, `offset`) marked `per_side` while their identical peers (`goblet-squat`, `halo`, `pullover`) were correctly `total`; the two `alternating-dumbbell-*` doubling a dumbbell they move one at a time; and `double-dumbbell-kickstand-deadlift` counting ×1 because `is_unilateral` was set truthfully about the STANCE. A name-based guard now fails on any reintroduction. (#224 #241) — done
