...
test('a false reading is confirmed instantly when it is not preceded by a resume', async () => {
  ...
  await rerender({ isSignedIn: false });
  expect(result.current).toBe(true);
});

test('a cold start that is already signed out confirms instantly', async () => {
  const getToken = jest.fn(async () => 'tok');
  const { result } = await renderHook(() => useResumeSignOutGuard(true, false, getToken));
  expect(result.current).toBe(true);
});

test('does nothing while Clerk has not loaded yet', async () => {
  const getToken = jest.fn(async () => 'tok');
  const { result } = await renderHook(() => useResumeSignOutGuard(false, false, getToken));
  expect(result.current).toBe(false);
});

test('a momentary false right after resume is held, not confirmed, and clears if it self-corrects', async () => {
  const getToken = jest.fn(async () => 'tok');
  const { result, rerender } = await renderHook(
    ({ isSignedIn }: { isSignedIn: boolean }) => useResumeSignOutGuard(true, isSignedIn, getToken),
    { initialProps: { isSignedIn: true } },
  ...
  await rerender({ isSignedIn: true });
  await act(() => {
...
test('a false reading that outlasts the resume window IS confirmed — a genuine sign-out discovered on resume still redirects', async () => {
  const getToken = jest.fn(async () => 'tok');
  const { result, rerender } = await renderHook(
    ({ isSignedIn }: { isSignedIn: boolean }) => useResumeSignOutGuard(true, isSignedIn, getToken),
    { initialProps: { isSignedIn: true } },
...
test('a resume nudges Clerk with a real, cache-skipping getToken call', async () => {
  const getToken = jest.fn(async () => 'tok');
  await renderHook(() => useResumeSignOutGuard(true, true, getToken));
...
test('a transition that is not a return from background does not nudge Clerk', async () => {
  const getToken = jest.fn(async () => 'tok');
  await renderHook(() => useResumeSignOutGuard(true, true, getToken));
 ...
 */
test('the AppState subscription is registered once, even as getToken is rebuilt every render', async () => {
  ...
  await rerender({ getToken: jest.fn(async () => 'tok2') });
  await rerender({ getToken: jest.fn(async () => 'tok3') });
 ...
 */
test('the confirm timer is cleared on unmount, so it cannot fire against an unmounted guard', async () => {
  const getToken = jest.fn(async () => 'tok');
 ...
 */
test('a timer that wakes after a later resume reschedules instead of confirming or hanging forever', async () => {
  const getToken = jest.fn(async () => 'tok');
  const { result, rerender } = await renderHook(
    ({ isSignedIn }: { isSignedIn: boolean }) => useResumeSignOutGuard(true, isSignedIn, getToken),
    { initialProps: { isSignedIn: true } },
