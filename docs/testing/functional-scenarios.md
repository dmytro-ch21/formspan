...
- Open `/food/target` on an account missing height, date of birth or sex: the button reads **Open profile** and lands on the profile form, which edits all three.
- Open it on an account whose profile is complete but which has never weighed in: the button reads **Record a weigh-in** and lands on today's check-in. It must NOT go to the profile — there is no weight field there.
- With both kinds missing, the profile wins. You have to go there anyway, and the weigh-in is one tap from Today afterwards.
- With nothing missing, no button renders at all.

### Edge cases & errors

- A field name this build does not know (server vocabulary ahead of the app) must not be treated as a profile field — it falls through to the weigh-in rather than silently routing to a form that cannot fix it.
- The check-in route needs a date; it is **today's**, because a check-in is written against the day it describes.

### Regression trap

- **Every navigation literal must name a route that exists**, and `lib/__tests__/routes.test.ts` is the only thing checking it. Expo Router's typed routes would catch this, but they are generated by Metro into a gitignored `.expo/`, so CI's `tsc` sees a loose `Href` and passes — the check is real locally and absent where it matters.
- **`profileGap` returns a kind, never an href.** Returning a path forces a cast at the call site, and that cast is exactly the type-checking that would have caught `/profile`. Keep the literals in the screen.
