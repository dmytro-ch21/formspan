 ...
 *
 * **Inlined rather than `<Image>`d, and drawn with `fill="currentColor"`.** The
 * letters take the ambient text colour rather than a hex baked into a file —
 * here that resolves to `--color-text` (`#111312`) via `body`, since nothing
 * between the body and the svg sets a colour. There is no second asset to keep
 * in sync, and it renders without a network request.
 *
 * **This console has one theme.** `apps/admin` has no `data-theme`, no
 * `prefers-color-scheme` and no `dark:` utilities, so `currentColor` buys
 * nothing today — it costs nothing either, and it means the wordmark is already
 * right if a dark palette ever lands. (`apps/web`'s copy of this file explains
 * the same choice against *its* light/dark toggle. Do not port that reasoning
 * back here; it describes machinery this app does not have.)
 *
 * One consequence worth knowing: because the colour is inherited rather than
 * declared, dropping the lockup inside anything that sets its own `color` — the
 * `bg-accent-dark` button style, a future dark header — retints the wordmark.
 * That is the intent, but it makes placement a colour decision as well as a
 * layout one.
 *
 ...
 *
 * **This file is a copy of `apps/web/src/app/Brand.tsx`, and that is a known
 * cost rather than an oversight.** The two apps are separate Next projects with
 * no shared package between them, and the agreed direction is a generator that
 * reads `logos/source/*.svg` and emits one module per app — at which point both
 * copies become build output and drift becomes a red CI run rather than
 * something review has to notice. Extracting a runtime `packages/brand` instead
 * would cut across that plan for the same end state. Until the generator lands:
 * **change one of these files and you must change the other.**
 */
