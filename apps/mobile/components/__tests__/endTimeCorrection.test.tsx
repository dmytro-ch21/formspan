...
describe('the collapsed row', () => {
  it('shows the current value, not "now"', async () => {
    await render(<EndTimeCorrection value={new Date(2026, 7, 20, 19, 0, 0)} now={() => NOW} onChange={onChange} testID="et" />);
    expect(screen.getByTestId('et-value')).toHaveTextContent('7:00 PM');
  });
});

describe('quick offset chips', () => {
  it('"Just now" sets the real now, regardless of the current value', async () => {
    await render(<EndTimeCorrection value={new Date(2026, 7, 20, 15, 0, 0)} now={() => NOW} onChange={onChange} testID="et" />);
    await fireEvent.press(screen.getByTestId('et-row'));
    await fireEvent.press(screen.getByTestId('et-offset-0'));
    expect(onChange).toHaveBeenCalledWith(NOW);
  });

  it('"1h ago" subtracts from NOW, not from the value already showing', async () => {
    ...
    await render(<EndTimeCorrection value={new Date(2026, 7, 20, 18, 0, 0)} now={() => NOW} onChange={onChange} testID="et" />);
    await fireEvent.press(screen.getByTestId('et-row'));
    await fireEvent.press(screen.getByTestId('et-offset-60'));
    expect(onChange).toHaveBeenCalledWith(new Date(NOW.getTime() - 60 * 60_000));
  });

  it('"2h ago" and "3h ago" both compute off NOW', async () => {
    await render(<EndTimeCorrection value={NOW} now={() => NOW} onChange={onChange} testID="et" />);

    await fireEvent.press(screen.getByTestId('et-row'));
    await fireEvent.press(screen.getByTestId('et-offset-120'));
    expect(onChange).toHaveBeenLastCalledWith(new Date(NOW.getTime() - 120 * 60_000));

    await fireEvent.press(screen.getByTestId('et-row'));
    await fireEvent.press(screen.getByTestId('et-offset-180'));
    expect(onChange).toHaveBeenLastCalledWith(new Date(NOW.getTime() - 180 * 60_000));
  });

  it('closes the sheet after a chip is tapped', async () => {
    await render(<EndTimeCorrection value={NOW} now={() => NOW} onChange={onChange} testID="et" />);
    await fireEvent.press(screen.getByTestId('et-row'));
    expect(screen.getByTestId('et-sheet')).toBeTruthy();
    await fireEvent.press(screen.getByTestId('et-offset-30'));
...
describe('fine-tune nudges', () => {
  it('nudge the DRAFT, not the committed value, until Save is pressed', async () => {
    await render(<EndTimeCorrection value={NOW} now={() => NOW} onChange={onChange} testID="et" />);
    await fireEvent.press(screen.getByTestId('et-row'));

    await fireEvent.press(screen.getByTestId('et-nudge-back'));
    await fireEvent.press(screen.getByTestId('et-nudge-back'));
    ...
    await fireEvent.press(screen.getByTestId('et-save'));
    expect(onChange).toHaveBeenCalledWith(new Date(NOW.getTime() - 30 * 60_000));
  });

  it('cancel discards nudges entirely', async () => {
    await render(<EndTimeCorrection value={NOW} now={() => NOW} onChange={onChange} testID="et" />);
    await fireEvent.press(screen.getByTestId('et-row'));
    await fireEvent.press(screen.getByTestId('et-nudge-forward'));
    await fireEvent.press(screen.getByTestId('et-cancel'));
    expect(onChange).not.toHaveBeenCalled();
  });

  it('reseeds the draft from the current value each time the sheet opens', async () => {
    ...
    await rerender(<EndTimeCorrection value={new Date(2026, 7, 20, 14, 0, 0)} now={() => NOW} onChange={onChange} testID="et" />);
    await fireEvent.press(screen.getByTestId('et-row'));
...
describe('a floor via notBefore', () => {
  ...
  it('disables a chip that would land before the floor, and it does nothing when tapped', async () => {
    await render(
    ...
    await fireEvent.press(hourAgo);
    expect(onChange).not.toHaveBeenCalled();
  });

  it('leaves a chip that lands exactly on the floor enabled', async () => {
    await render(
      <EndTimeCorrection value={NOW} now={() => NOW} notBefore={FLOOR} onChange={onChange} testID="et" />,
    );
    await fireEvent.press(screen.getByTestId('et-row'));
    ...
    await fireEvent.press(halfHourAgo);
    expect(onChange).toHaveBeenCalledWith(FLOOR);
  });

  it('nudging earlier never crosses the floor, no matter how many taps', async () => {
    await render(
      <EndTimeCorrection value={NOW} now={() => NOW} notBefore={FLOOR} onChange={onChange} testID="et" />,
    );
    await fireEvent.press(screen.getByTestId('et-row'));
  ...
  it('save clamps even a value seeded below the floor (the caller already had an invalid one)', async () => {
  ...
  it('never disables the "later" nudge — only earlier can cross a floor', async () => {
    await render(
      <EndTimeCorrection value={NOW} now={() => NOW} notBefore={FLOOR} onChange={onChange} testID="et" />,
    );
    await fireEvent.press(screen.getByTestId('et-row'));
  ...
  it('with no notBefore prop, nothing is disabled and offsets are never clamped', async () => {
    await render(<EndTimeCorrection value={NOW} now={() => NOW} onChange={onChange} testID="et" />);
    await fireEvent.press(screen.getByTestId('et-row'));
    await fireEvent.press(screen.getByTestId('et-offset-240')); // 4h ago — well past FLOOR, but no floor set
