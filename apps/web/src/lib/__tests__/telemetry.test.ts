...
/**
 * The web copy of the buffer's tests.
 *
 * Duplicated alongside the buffer itself, and `check:telemetry-parity` guards
 * the SOURCE rather than these — deliberately. A test file that must also stay
 * byte-identical is a second thing to keep in step for no extra safety: the
 * guarantee comes from the two implementations agreeing, and each suite proves
 * its own copy behaves. If the sources match and both suites pass, both copies
 * are right.
 *
