...
cd backend && go run ./cmd/exportcontent    # carry console-authored content into BOTH files of BOTH catalogs — techniques.json/.additions.json and exercises.json/.additions.json. One without the other is lost: the deploy will not carry it, or the next spreadsheet re-import deletes it
