...
4. **`backend/migrations/YYYYMMDDHHMMSS_<description>.up.sql` + `.down.sql`** — plain SQL, versioned by the current UTC timestamp to the second (`date -u +%Y%m%d%H%M%S`), generated fresh right before writing the file — not the next sequential number (CLAUDE.md's "Git / PR workflow" section, N149/#553: a shared counter collides across concurrent agents, a timestamp does not). Existing `NNNNNN`-numbered migrations are historical and untouched.
