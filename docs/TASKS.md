...
- [ ] **N15** — The search synonym map is hand-maintained lifting vocabulary (`exercise/search.go`). It is small and closed by design — it grows when a word is ambiguous in the gym, never per exercise — but nothing checks it against the catalog, so a synonym pointing at a word no row contains is silently dead. (#PR)
