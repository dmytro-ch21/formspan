...
- [x] **W5** — The lunge / split-squat / step-up family disagrees with itself about implement count, and at least one side of every pair is wrong. Every **kettlebell** lunge doubles; every **dumbbell** lunge except `dumbbell-lunge` does not — same movement, opposite factor by equipment. Worse within one implement: `kettlebell-split-squat` ×2 vs `kettlebell-bulgarian-split-squat` ×1. Provable from internal inconsistency alone, but which side is right is a product call (is a dumbbell lunge one dumbbell or two?), so it wants a decision before ~14 rows move athletes' tonnage. Found by review of #254. — done #PR. **The rule was wrong, not the rows**: the factor was derived from `load_mode AND NOT is_unilateral`, reading "one LIMB" as "one IMPLEMENT", so a dumbbell walking lunge (two implements, one leg) could not be expressed and whoever classified it had to pick which error to ship. New `exercises.implements` carries the factor; `is_unilateral` goes back to meaning only the reps hint. 8 rows' tonnage changes. (#254)
...
- [ ] **W6** — The rest of the catalog has never been re-read against the *separated* facts. `implements` was backfilled from the old derived rule, so every row outside the lunge family inherits whatever that rule said — including any other movement held with two implements while working one limb (carries, some presses). The lunge family was found by internal contradiction; nothing has swept for the rest. (#PR)
