...
describe('FoodSummaryCard', () => {
  it('states the item count, total calories and macro split for a real day', async () => {
    ...
    await render(<FoodSummaryCard eaten={eaten} testID="food-summary" />);
  ...
  it('a genuine zero — nobody has logged yet — is a real, honest answer, not hidden', async () => {
    const eaten: EatenView = {
      state: 'ready',
      rows: [],
  ...
  it('singular "item" for exactly one entry', async () => {
    const eaten: EatenView = {
      state: 'ready',
      rows: [entry()],
  ...
  it('renders nothing while loading or unavailable — a read that has not happened is not a zero', async () => {
    await render(<FoodSummaryCard eaten={{ state: 'loading' }} testID="food-summary" />);
    expect(screen.queryByTestId('food-summary')).toBeNull();

    await render(<FoodSummaryCard eaten={{ state: 'unavailable' }} testID="food-summary" />);
    expect(screen.queryByTestId('food-summary')).toBeNull();
  });
});
