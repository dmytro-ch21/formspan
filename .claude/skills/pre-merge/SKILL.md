...
Give each reviewer the actual diff scope *and the design intent* behind the
change, not just "review this branch". They find substantially more when
they know what the code is trying to do and which properties are
load-bearing — say which invariant matters, and what would be a disaster if
it broke.
...
running the checks *feels* like having verified the change. It isn't. The
checks prove it compiles and that CI will pass. They have never once caught
an authorization gap; the reviewers have caught several, including the same
cross-user ID-enumeration bug twice, in two different modules.
