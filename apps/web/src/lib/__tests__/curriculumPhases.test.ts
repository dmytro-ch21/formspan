...
describe("groupByPhase", () => {
  ...
  it("keeps empty phases, which are authoring in progress rather than nothing", () => {
    const groups = groupByPhase(phases, [item({ technique_id: "a", phase: 0 })]);
    expect(groups.map((g) => g.items.length)).toEqual([1, 0]);
  });

  it("falls back to unphased for a dangling phase index instead of dropping the item", () => {
    const groups = groupByPhase(phases, [item({ technique_id: "a", phase: 7 })]);
    const all = groups.flatMap((g) => g.items);
    expect(all.map((i) => i.technique_id)).toContain("a");
    expect(groups[0].phase).toBeNull();
  });
});
